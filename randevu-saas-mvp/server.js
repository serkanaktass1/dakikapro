...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (pathname.startsWith('/api/')) {
      ...
      if (req.method === 'GET' && (params=match('/api/public/:slug/display', pathname))) { const t=db.tenants.find(x=>x.slug===params.slug && x.status!=='inactive'); if(!t) return json(res,404,{error:'İşletme bulunamadı'}); if(!['pro','business'].includes(t.planId)) return json(res,403,{error:'TV randevu panosu Profesyonel ve İşletme paketlerinde kullanılabilir.'}); const d=tenantData(db,t.id),todayKey=new Date().toISOString().slice(0,10),mask=name=>{const p=String(name||'Misafir').trim().split(/\s+/);return p.length>1?`${p[0]} ${p[p.length-1][0]}.`:p[0]}; const appointments=d.appointments.filter(item=>item.date===todayKey&&item.status!=='cancelled').sort((a,b)=>a.time.localeCompare(b.time)).map(item=>({time:item.time,status:item.status,customer:mask(d.customers.find(c=>c.id===item.customerId)?.name),service:d.services.find(s=>s.id===item.serviceId)?.name||'Hizmet',staff:d.staff.find(s=>s.id===item.staffId)?.name||'Uzman'})); return json(res,200,{tenant:{name:t.name,sector:t.sector,color:t.color||'#4f46e5'},appointments,updatedAt:new Date().toISOString()}); }
