  ...
  try {
    if (pathname.startsWith('/api/')) {
      ...
      if (req.method === 'PATCH' && (params=match('/api/public/:slug/appointments/:appointmentId', pathname))) {
        ...
        if(body.status==='cancelled') { a.status='cancelled';a.customerAction='cancelled';a.cancelledAt=new Date().toISOString(); }
        else if(body.date&&body.time) { const holidays=String(t.holidays||'').split(',').map(x=>x.trim()).filter(Boolean); if(holidays.includes(body.date))return json(res,400,{error:'Bu tarih için işletme kapalı. Lütfen başka bir gün seçin.'}); const clash=db.appointments.some(x=>x.id!==a.id&&x.tenantId===t.id&&x.staffId===a.staffId&&x.date===body.date&&x.time===body.time&&x.status!=='cancelled'); if(clash)return json(res,409,{error:'Bu saat artık dolu. Lütfen başka bir saat seçin.'}); a.previousDate=a.date;a.previousTime=a.time;a.date=body.date;a.time=body.time;a.status='pending';a.customerAction='rescheduled';a.rescheduledAt=new Date().toISOString();a.note='Müşteri tarih/saat değişikliği talebi'; }
        else return json(res,400,{error:'Geçerli bir değişiklik seçin.'}); writeDb(db); return json(res,200,a);
      }
      if (req.method === 'GET' && (params=match('/api/public/:slug', pathname))) { const t=db.tenants.find(x=>x.slug===params.slug && x.status!=='inactive'); if(!t) return json(res,404,{error:'İşletme bulunamadı'}); const d=tenantData(db,t.id); const busySlots=d.appointments.filter(item=>item.status!=='cancelled').map(item=>({staffId:item.staffId,date:item.date,time:item.time})); return json(res,200,{tenant:t,staff:d.staff,services:d.services,busySlots}); }
      if (req.method === 'GET' && (params=match('/api/public/:slug/display', pathname))) { const t=db.tenants.find(x=>x.slug===params.slug && x.status!=='inactive'); if(!t) return json(res,404,{error:'İşletme bulunamadı'}); if(!t.tvDisplayEnabled) return json(res,403,{error:'TV randevu panosu bu işletme için henüz açık değil.'}); const d=tenantData(db,t.id),todayKey=new Date().toISOString().slice(0,10),mask=name=>{const p=String(name||'Misafir').trim().split(/\s+/);return p.length>1?`${p[0]} ${p[p.length-1][0]}.`:p[0]}; const appointments=d.appointments.filter(item=>item.date===todayKey&&item.status==='confirmed').sort((a,b)=>a.time.localeCompare(b.time)).map(item=>({id:item.id,time:item.time,status:item.status,customer:mask(d.customers.find(c=>c.id===item.customerId)?.name),service:d.services.find(s=>s.id===item.serviceId)?.name||'Hizmet',staff:d.staff.find(s=>s.id===item.staffId)?.name||'Uzman'})); return json(res,200,{tenant:{name:t.name,sector:t.sector,color:t.color||'#4f46e5'},appointments,updatedAt:new Date().toISOString()}); }
      ...
      if (req.method === 'POST' && (params=match('/api/tenant/:tenantId/:collection',pathname))) { const body=await parseBody(req); const valid=['staff','services','customers','appointments']; if(!valid.includes(params.collection))return json(res,404,{error:'Geçersiz koleksiyon'}); if(params.collection==='appointments'&&body.note==='Online randevu talebi'){const tenant=db.tenants.find(item=>item.id===params.tenantId),hours=String(tenant?.workingHours||'').match(/(\d{1,2}:\d{2})\s*[–-]\s*(\d{1,2}:\d{2})/),start=hours?hours[1].padStart(5,'0'):'09:00',end=hours?hours[2].padStart(5,'0'):'19:00';if(!body.time||body.time<start||body.time>=end)return json(res,400,{error:`Bu işletme ${start}–${end} saatleri arasında randevu kabul ediyor.`});body.status=tenant?.appointmentApprovalMode==='auto'?'confirmed':'pending';} const singular={staff:'s',services:'v',customers:'c',appointments:'a'}[params.collection]; const item={...body,id:id(singular),tenantId:params.tenantId}; db[params.collection].push(item); writeDb(db); return json(res,201,item); }
