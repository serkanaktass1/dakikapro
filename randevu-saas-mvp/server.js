...
function seed() {
  ...
  return {
    ...
    plans: [
      { id: 'starter', name: 'Başlangıç', price: 299, yearlyPrice: 2990, staffLimit: 2, featured: false, features: ['Online randevu sayfası', '2 çalışan', 'Hizmet ve müşteri yönetimi', 'Temel randevu takvimi'] },
      { id: 'pro', name: 'Profesyonel', price: 599, yearlyPrice: 5990, staffLimit: 8, featured: true, features: ['Başlangıç’taki her şey', '8 çalışan', 'Randevu durumu yönetimi', 'Müşteri geçmişi ve notlar', 'Öncelikli destek'] },
      { id: 'business', name: 'İşletme', price: 999, yearlyPrice: 9990, staffLimit: 30, featured: false, features: ['Profesyonel’deki her şey', '30 çalışan', 'Çok şubeli yapıya hazır altyapı', 'Gelişmiş raporlama', 'Özel destek'] }
    ],
    tenants: [
    ...
    admins: [
      { id: 'admin_1', name: 'Serkan Aktaş', email: 'sa@dadikapro.com', passwordHash: hashPassword('Serkanaktass1') }
    ],
    appointments: [
...
function readDb() { ensureDb(); const db=JSON.parse(fs.readFileSync(dataFile, 'utf8')); let changed=false;if(!db.owners){db.owners=[{id:'o_demo',tenantId:'t_demo',name:'Ahmet Yılmaz',email:'owner@makasberber.com',passwordHash:hashPassword('demo123')}];changed=true}if(!db.admins){db.admins=[{id:'admin_1',name:'Serkan Aktaş',email:'sa@dadikapro.com',passwordHash:'367dc7290c4575ab0e9847869af3952af0007f8ef2b1799df065002d534582de'}];changed=true}Object.assign(db.settings,{supportEmail:'destek@dakikapro.com',supportPhone:'0850 000 00 00',paymentProvider:'iyzico',allowOnlinePayment:true,allowBankTransfer:true,iban:'',accountHolder:'',maintenanceMode:false,notifications:{trial:true,payment:true,appointment:true}},db.settings);if(db.settings.platformName==='RandevuPro'){db.settings.platformName='DakikaPro';changed=true}if(db.settings.supportEmail==='destek@randevupro.com'){db.settings.supportEmail='destek@dakikapro.com';changed=true}for(const p of db.plans){if(!p.yearlyPrice){p.yearlyPrice=Math.round(p.price*10);changed=true}}for(const t of db.tenants){if(!t.billingCycle){t.billingCycle='monthly';changed=true}if(!t.workingHours){t.workingHours='Pazartesi–Cumartesi, 09:00–19:00';changed=true}}if(changed)writeDb(db); return db; }
